...
class InvertedResidual(nn.Module):
    ...
    def __init__(
        ...
        out_channels: int,
        stride: int,
        expand_ratio: Union[int, float],    # 经过第一个1×1的conv2d，通道数翻几倍
        skip_connection: Optional[bool] = True,  # 是否使用残差分支
    ) -> None:
        assert stride in [1, 2]
        hidden_dim = make_divisible(int(round(in_channels * expand_ratio)), 8)  # 经过第一个1×1的conv2d后的通道数
        ...
        block.add_module(
            ...
            module=ConvLayer(
                ...
                out_channels=out_channels,
                kernel_size=1,
                use_act=False,   # 不使用 RELU6（默认使用）
                use_norm=True,   # 使用 BN（默认使用）
            ),
        )
...
class MobileViTBlock(nn.Module):
    ...
    def __init__(
        self,
        in_channels: int,
        transformer_dim: int,           # 输入到transformer每个token的长度
        ffn_dim: int,                   # transformer的MLP中第一个全连接层结点的个数
        n_transformer_blocks: int = 2,  # 重复堆叠transformer block的个数
        head_dim: int = 32,
        ...
        patch_h: int = 8,
        patch_w: int = 8,
        conv_ksize: Optional[int] = 3,  # Local representation中卷积核的大小
        *args,
        **kwargs
    ) -> None:
        super().__init__()
...
class MobileViT(nn.Module):
    ...
    @staticmethod
    def _make_mit_layer(input_channel: int, cfg: Dict) -> list[nn.Sequential, int]:
